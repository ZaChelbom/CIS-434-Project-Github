...
func _process(delta: float) -> void:
	if card_being_dragged: #if there is a card being dragged
		var mouse_pos = get_global_mouse_position()
		card_being_dragged.position = Vector2(clamp(mouse_pos.x, 0, screen_size.x), 
			clamp(mouse_pos.y, 0, screen_size.y)) #have the card follow the mouse
...
func get_card_with_highest_z_index(cards):
	#Assume first card in cards array has the highest z index
...
func on_hovered_off_card(card):
	if !card_being_dragged:
		...
		#check if hovered off card and straight on to another card
